...
const ref='TEST-SETTLE-06371-20260907';
...
const matches=[...bills.matchAll(/<BILL NAME=[\s\S]*?<\/BILL>/g)].map(m=>m[0]).filter(b=>b.includes(party));
if(matches.length!==1)throw Error('Ambiguous bill');
const closing=Number(matches[0].match(/<CLOSINGBALANCE[^>]*>([^<]+)</)?.[1]);
if(!Number.isFinite(closing)||closing>=0)throw Error('Balance is already at or below target; refusing write');
if(Math.round(-closing*100)!==1000000)throw Error('Expected exactly 10000 remaining; refusing changed balance');
const amount=(-closing).toFixed(2);
console.log('RECEIPT AMOUNT',amount,'TARGET OUTSTANDING',0);
if(process.argv.includes('--execute')){
 const xml=`<ENVELOPE><HEADER><TALLYREQUEST>Import Data</TALLYREQUEST></HEADER><BODY><IMPORTDATA><REQUESTDESC><REPORTNAME>Vouchers</REPORTNAME><STATICVARIABLES><SVCURRENTCOMPANY>${company}</SVCURRENTCOMPANY></STATICVARIABLES></REQUESTDESC><REQUESTDATA><TALLYMESSAGE><VOUCHER VCHTYPE="Receipt" ACTION="Create"><DATE>20260907</DATE><VOUCHERTYPENAME>Receipt</VOUCHERTYPENAME><REFERENCE>${ref}</REFERENCE><NARRATION>TEST partial receipt for payment follow-up UI verification - ${ref}</NARRATION><ALLLEDGERENTRIES.LIST><LEDGERNAME>${esc(bank)}</LEDGERNAME><ISDEEMEDPOSITIVE>Yes</ISDEEMEDPOSITIVE><AMOUNT>-${amount}</AMOUNT></ALLLEDGERENTRIES.LIST><ALLLEDGERENTRIES.LIST><LEDGERNAME>${esc(party)}</LEDGERNAME><ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE><ISPARTYLEDGER>Yes</ISPARTYLEDGER><AMOUNT>${amount}</AMOUNT><BILLALLOCATIONS.LIST><NAME>${bill}</NAME><BILLTYPE>Agst Ref</BILLTYPE><AMOUNT>${amount}</AMOUNT></BILLALLOCATIONS.LIST></ALLLEDGERENTRIES.LIST></VOUCHER></TALLYMESSAGE></REQUESTDATA></IMPORTDATA></BODY></ENVELOPE>`;
