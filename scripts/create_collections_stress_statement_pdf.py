...
def build_pdf(manifest_path: Path) -> Path:
    ...
    story = [
        Paragraph("<b>Kotak Mahindra Bank</b>", title),
        Paragraph("Account Statement", heading),
        Paragraph(
        ...
        Paragraph(f"Customer: {manifest['accountHolderName']}", normal),
        Spacer(1, 8),
    ]
    ...
    story.append(Paragraph("This statement is system generated and does not require a signature.", normal))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("Usage: create_collections_stress_statement_pdf.py <manifest.json>")
