...
export function readReminderBills(request:Request, scope:{connectionId:string;companyName:string;financialYear:string;organizationId:string}, ledger:string, verificationInvoice?:{invoice:string;invoiceDate:string}) {
  ...
  return new Promise<Record<string,unknown>>((resolve,reject)=>{
    ...
    const finish=(error?:Error,data?:Record<string,unknown>)=>{if(done)return;done=true;clearTimeout(timer);socket.close();error?reject(error):resolve(data!);};
    ...
    socket.addEventListener('message',event=>{
      try {
        ...
        if(!requested&&(message.type==='authenticated'||(message.type==='hello'&&message.authenticated===true))) {requested=true;socket.send(JSON.stringify({type:'request',requestId:id,...scope,operation:'fetch_customer_open_bills',payload:{companyName:scope.companyName,financialYear:scope.financialYear,ledgerNames:[ledger],queryPurpose:'payment_followup',verificationInvoice}}));}
        else if(message.type==='error') finish(new Error(message.error||'Tally verification failed.'));
        else if(message.type==='result'&&message.requestId===id) message.success?finish(undefined,message.data):finish(new Error(message.error||'Tally verification failed.'));
